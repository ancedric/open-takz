...
export const useUserStore = defineStore('user', () => {
  ...
  const init = async () => {
    ...
    try {
      ...
      const userRes = await axios.get(`${api}/user/${_sessionRaw.userref}`)
      if(userRes.error){
        Alert(userRes.message)
        throw userRes.error;
      }

      const empRes = await axios.get(`${api}/employe/get-employe/${userRes.data.userref}`)
      if(empRes.error){
        Alert(empRes.message)
        throw empRes.error;
      }

      const compRes = await axios.get(`${api}/company/get-company/${empRes.companyref}`)
      if(compRes.error){
        Alert(compRes.message)
        throw compRes.error;
      }
